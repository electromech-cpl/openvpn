...
import (
	...
	"github.com/d3vilh/ElectromechVPN-ui/state"
)
			...
			cmd := exec.Command("/bin/bash", "-c",
				fmt.Sprintf(
					"cd /opt/scripts/ && "+
						...
						"export EASYRSA_REQ_OU=%s &&"+
						"./genclient.sh %s %s &&"+
						"echo 'ifconfig-push %s 255.255.255.0' > /etc/ElectromechVPN/staticclients/%s", name, tfaname, tfaissuer, expiredays, email, country, province, city, org, orgunit, name, staticip, staticip, name))
			cmd.Dir = state.GlobalCfg.OVConfigPath
			output, err := cmd.CombinedOutput()
			if err != nil {
				logs.Debug(string(output))
			...
			cmd := exec.Command("/bin/bash", "-c",
				fmt.Sprintf(
					"cd /opt/scripts/ && "+
						...
						"export EASYRSA_REQ_OU=%s &&"+
						"./genclient.sh %s %s %s &&"+
						"echo 'ifconfig-push %s 255.255.255.0' > /etc/ElectromechVPN/staticclients/%s", name, tfaname, tfaissuer, expiredays, email, country, province, city, org, orgunit, name, staticip, passphrase, staticip, name))
			cmd.Dir = state.GlobalCfg.OVConfigPath
			output, err := cmd.CombinedOutput()
			if err != nil {
				logs.Debug(string(output))
