...
<img src="https://raw.githubusercontent.com/d3vilh/ElectromechVPN-ui/main/docs/images/ElectromechVPN-UI-Home.png" alt="Openvpn-ui home screen"/>

[![latest version](https://img.shields.io/github/v/release/d3vilh/ElectromechVPN-ui?color=%2344cc11&label=LATEST%20RELEASE&style=flat-square&logo=Github)](https://github.com/d3vilh/ElectromechVPN-ui/releases/latest) [![Docker Image Version (tag latest semver)](https://img.shields.io/docker/v/d3vilh/ElectromechVPN-ui/latest?logo=docker&label=DOCKER%20IMAGE&color=2344cc11&style=flat-square&logoColor=white)](https://hub.docker.com/r/d3vilh/ElectromechVPN-ui) ![Docker Image Size (tag)](https://img.shields.io/docker/image-size/d3vilh/ElectromechVPN-ui/latest?logo=Docker&color=2344cc11&label=IMAGE%20SIZE&style=flat-square&logoColor=white)

[![latest version](https://img.shields.io/github/v/release/d3vilh/ElectromechVPN-server?color=%2344cc11&label=OpenVPN-SERVER&style=flat-square&logo=Github)](https://github.com/d3vilh/ElectromechVPN-server)  [![Docker Image Version (tag latest semver)](https://img.shields.io/docker/v/d3vilh/ElectromechVPN-server/latest?style=flat-square&logo=docker&logoColor=white&label=OpenVPN-SERVER%20IMAGE&color=2344cc11)](https://hub.docker.com/r/d3vilh/ElectromechVPN-server)
...
* Two factor authentication (**[2FA/MFA](https://github.com/d3vilh/ElectromechVPN-ui#two-factor-authentication-2fa)**) support
...
* **ElectromechVPN-UI users management**. Administrators has full access, regular users to Certificates management, logs and status page only.
* ElectromechVPN-UI Admin user and password can be passed via environment variables to container
* Updated infrustracture:
  ...
  * OpenVPN 2.5.8 Server is fully compatible
    * Compatible OpenVPN Server images can be found on Docker Hub - [d3vilh/ElectromechVPN-server:latest](https://hub.docker.com/r/d3vilh/ElectromechVPN-server)
    * As well as Openvpn-UI itself - [d3vilh/ElectromechVPN-ui:latest](https://hub.docker.com/r/d3vilh/ElectromechVPN-ui)
* Support any architecture, ready images for AMD64 and ARM [available on Docker Hub](https://hub.docker.com/r/d3vilh/ElectromechVPN-ui).

Part of following projects:
* [Openvpn-aws](https://github.com/d3vilh/ElectromechVPN-aws) OpenVPN and ElectromechVPN-UI for any Cloud, VM or x86 bare metal server.
...
For the best experience, it is recommended to deploy it within a Docker environment consisting of two distinct containers:
 - The [`d3vilh/ElectromechVPN-server`](https://github.com/d3vilh/ElectromechVPN-server) Back-End container (ElectromechVPN) for running OpenVPN server.
 - OpenVPN UI Front-End container (ElectromechVPN-ui) for efficient management of the OpenVPN server environment.

However it works fine as standalone application with standalone OpenVPN server as well.
### Intel x86 and AMD64 platforms
You can run both containers from the official [ElectromechVPN-server](https://github.com/d3vilh/ElectromechVPN-server) repository, `docker-compose-ElectromechVPNui.yml`.
It includes all the files in its main directory. 

For automated installation on baremetal x86-64 servers, Cloud or VM installation, please use [ElectromechVPN-aws](https://github.com/d3vilh/ElectromechVPN-aws) project.
It, as well, includes all the necessary scripts for easy installation of ElectromechVPN-UI and OpenVPN server on any x86-64 platform.
...
You can run both containers from the [ElectromechVPN-server](https://github.com/d3vilh/ElectromechVPN-server) repository as well (use `docker-compose-ElectromechVPNui.yml`).
...
```yaml
    ElectromechVPN-ui:
       container_name: ElectromechVPN-ui
       image: d3vilh/ElectromechVPN-ui:latest
       environment:
...
You can also couple ElectromechVPN-UI with recommended [d3vilh/ElectromechVPN-server](https://github.com/d3vilh/ElectromechVPN-server) image and here is updated `docker-compose.yml` for it:
...
services:
    ElectromechVPN:
       container_name: ElectromechVPN
       image: d3vilh/ElectromechVPN-server:latest
       privileged: true
       ports: 
          - "1194:1194/udp"
       environment:
       ...
       volumes:
           - ./pki:/etc/ElectromechVPN/pki
           - ./clients:/etc/ElectromechVPN/clients
           - ./config:/etc/ElectromechVPN/config
           - ./staticclients:/etc/ElectromechVPN/staticclients
           - ./log:/var/log/ElectromechVPN
           - ./fw-rules.sh:/opt/app/fw-rules.sh
           - ./server.conf:/etc/ElectromechVPN/server.conf
       cap_add:
           - NET_ADMIN
       restart: always
...
```shell
mkdir -p /var/data/d3ElectromechVPN # change it as you needed
cd /var/data/d3ElectromechVPN

git clone https://github.com/d3vilh/ElectromechVPN-server . # or download the zip file from GitHub if you don't have git installed
...
mkdir -p ./config
wget https://raw.githubusercontent.com/d3vilh/ElectromechVPN-server/refs/heads/main/server.conf -O server.conf
wget https://raw.githubusercontent.com/d3vilh/ElectromechVPN-server/refs/heads/main/config/easy-rsa.vars -O config/easy-rsa.vars
wget https://raw.githubusercontent.com/d3vilh/ElectromechVPN-server/refs/heads/main/config/client.conf -O config/client.conf
echo "" > ./fw-rules.sh
...
```shell
~/ElectromechVPN-server $ cat fw-rules.sh
...
Run the ElectromechVPN-UI image
```shell
docker run \
-v /home/pi/ElectromechVPN:/etc/ElectromechVPN \
-v /home/pi/ElectromechVPN/db:/opt/ElectromechVPN-ui/db \
-v /home/pi/ElectromechVPN/pki:/usr/share/easy-rsa/pki \
-v /home/pi/ElectromechVPN/log:/var/log/ElectromechVPN \
-v /var/run/docker.sock:/var/run/docker.sock \
-e OPENVPN_ADMIN_USERNAME='admin' \
-e OPENVPN_ADMIN_PASSWORD='gagaZush' \
-p 8080:8080/tcp \
--privileged d3vilh/ElectromechVPN-ui:latest
```

Run the OpenVPN Server image:
```shell
git clone https://github.com/d3vilh/ElectromechVPN-server ~/ElectromechVPN-server && \
cd ~/ElectromechVPN-server/ && \
docker run  --interactive --tty --rm \
  --name=ElectromechVPN-server \
  --cap-add=NET_ADMIN \
  -p 1194:1194/udp \
  -e TRUST_SUB=10.0.70.0/24 \
  -e GUEST_SUB=10.0.71.0/24 \
  -e HOME_SUB=192.168.88.0/24 \
  -v ./pki:/etc/ElectromechVPN/pki \
  -v ./clients:/etc/ElectromechVPN/clients \
  -v ./config:/etc/ElectromechVPN/config \
  -v ./staticclients:/etc/ElectromechVPN/staticclients \
  -v ./log:/var/log/ElectromechVPN \
  -v ./fw-rules.sh:/opt/app/fw-rules.sh \
  -v ./server.conf:/etc/ElectromechVPN/server.conf \
  --privileged d3vilh/ElectromechVPN-server:latest
```
  </details>
...
##### Building the image
To build the ElectromechVPN-UI image:
```shell
cd build; ./build_ElectromechVPN-ui.sh
```
The new image will have `ElectromechVPN-ui` name.
...
If you have your OpenVPN server up and running on the same host, you can install ElectromechVPN-UI as standalone application.
For this you need to have GoLang installed on your host to build all the necessary binaries on the server itself.

To make installation easier, version 0.9.5 now includes a [`standalone-install.sh`](https://github.com/d3vilh/ElectromechVPN-ui/blob/e2f452d2872a022147f8c58213fa4306f61e65e8/build/standalone-install.sh) script. This script helps you step by step, starting with downloading and installing GoLang, and ending with building the binaries.

At the moment script supports Debian based systems only.

Here is installation process example:
```bash
superdude@bookworm64:~/build/ElectromechVPN-ui/build$ ./standalone-install.sh
This script will install ElectromechVPN-UI and all the dependencies on your local environment. No containers will be used.
...
Would you like to build ElectromechVPN-UI and install qrencode? (y/n) y
Installing ElectromechVPN-UI and qrencode
Cloning qrencode into build directory
Cloning into 'qrencode'...
remote: Enumerating objects: 35, done.
Resolving deltas: 100% (8/8), done.
Building and packing ElectromechVPN-UI
...
All done.
superdude@bookworm64:~/build/ElectromechVPN-ui/build$ 
```
After build and installation is complete:
1. Put archive `ElectromechVPN-ui.tar.gz` with binaries to the desired directory where you would like to run ElectromechVPN-UI from.
2. Uncompress it with `tar -xzf ElectromechVPN-ui.tar.gz`.
3. Double check ElectromechVPN-ui binary has +x permissions for your user.
4. Create `db` directory in the same directory where `ElectromechVPN-ui` binary is located.
5. Update `OpenVpnPath` and `EasyRsaPath` with real location of your OpenVPN server config (`/etc/ElectromechVPN`) and EasyRSA (`/usr/share/easy-rsa`) in main application config file - `./conf/app.conf` file. 
...
```
This should return your username and password you will use for the first login into UI. It is mandatory to set this vars on the first application start only. So ElectromechVPN-UI will create your Admin user with your own credentials.

9. Run `./ElectromechVPN-ui` binary and login with your new credentials.
...
During the installtion or upgrade process ElectromechVPN-UI by itself does not do any changes to your OpenVPN server configuration or PKI infrastructure. However it is recommended to perform backup of your `PKI infrastructure`, `server.conf`, `client.conf` and `data.db` before following with upgrade steps.

  <details>
    <summary>Backup</summary>

#### Backup
To backup your PKI infrastructure, server, client configuration files and ElectromechVPN-UI DB you can use `backup.sh` script which is in [`build/assets` directory](build/assets/backup.sh) (since the release `0.6`), it is also part of [ElectromechVPN-aws](https://github.com/d3vilh/ElectromechVPN-aws) and [Raspberry-Gateway](https://github.com/d3vilh/raspberry-gateway) projects (right in ElectromechVPN-server directory).
...
Then run the script:
```shell
sudo ./backup.sh -b ~/ElectromechVPN-server backup/ElectromechVPN-server-030923-1
```
this will create backup of all necessary files, from `~/ElectromechVPN-server` to `~/backup/ElectromechVPN-server-030923-1`.
...
#### Upgrade
To upgrade ElectromechVPN-UI to the latest version, you have to save old container image, remove old container and deploy new container with upgraded image.

##### Preparation
1. Check which ElectromechVPN-UI version image is currently used:
```shell
docker inspect --format='{{json .Config.Labels}}' d3vilh/ElectromechVPN-ui:latest
...
```shell
docker tag d3vilh/ElectromechVPN-ui:latest local/ElectromechVPN-ui:backup
```
...
```shell
admin@aws3:~/ElectromechVPN $ cat docker-compose.yml | grep image
       image: d3vilh/ElectromechVPN-ui:latest
admin@aws3:~/ElectromechVPN $
```
...
```shell
docker pull d3vilh/ElectromechVPN-ui:latest
```
...
```shell
docker rm ElectromechVPN-ui --force
```
...
```shell
admin@aws3:~/ElectromechVPN $ docker logs ElectromechVPN-ui
...
2023/09/03 12:38:50.650 [I] [server.go:280]  http server Running on http://:8080
admin@aws3:~/ElectromechVPN $

admin@aws3:~/ElectromechVPN $ docker logs ElectromechVPN
...
Start ElectromechVPN process...
admin@aws3:~/ElectromechVPN $
```

##### Verification process
Now when new ElectromechVPN-UI version is deployed, the DB schema were updated to the latest version automatically during the container start.
...
<img src="https://raw.githubusercontent.com/d3vilh/ElectromechVPN-ui/main/docs/images/ElectromechVPN-UI-Upgrade.01.png" alt="Openvpn-ui upgrade" width="500" border="1"/>
...
<img src="https://raw.githubusercontent.com/d3vilh/ElectromechVPN-ui/main/docs/images/ElectromechVPN-UI-Upgrade.02.png" alt="Openvpn-ui upgrade" width="500" border="1"/>

Here is how it should looks like:

<img src="https://raw.githubusercontent.com/d3vilh/ElectromechVPN-ui/main/docs/images/ElectromechVPN-UI-Upgrade.03.png" alt="Openvpn-ui upgrade" width="500" border="1"/>
...
And you are done with the upgrade process.
  ...
  | 0.4     | settings          | easy_r_s_a_path               | Configuration > ElectromechVPN-UI      |
...
1. Stop and remove updated ElectromechVPN-ui container:
```shell
docker rm ElectromechVPN-ui --force
```
2. Remove updated ElectromechVPN-ui image:
```shell
docker image rm d3vilh/ElectromechVPN-ui:latest
```
3. Restore old ElectromechVPN-ui image:
```shell
docker tag local/ElectromechVPN-ui:backup d3vilh/ElectromechVPN-ui:latest
```
...
```shell
sudo ./backup.sh -r ~/ElectromechVPN-server backup/ElectromechVPN-server-030923-1
```
This will restore all the enviroment files from backup directory to `~/ElectromechVPN-server` directory.

> **Note v.0.3**: There was bug in version 0.3 where data.db file were not shared over the volume, so you have to restore it manually: `docker cp backup/data.0.3.db ElectromechVPN-ui:/opt/ElectromechVPN-ui/data.db`
...
## Configuration
**OpenVPN UI** can be accessed on own port (*e.g. http://localhost:8080), the default user and password is `admin/gagaZush` preconfigured in `config.yml` if you are using Raspberry-Gateway or Openvpn-aws projects. For standalone installation, you can pass your own credentials via environment variables to container (refer to [Manual installation](https://github.com/d3vilh/ElectromechVPN-ui#manual-installation)).

### Container volume
The container volume can be initialized by using the [d3vilh/ElectromechVPN-server](https://github.com/d3vilh/ElectromechVPN-server) image with included scripts to automatically generate everything you need on the first run:
...
If you are running ElectromechVPN-UI as manuall installation, please be sure `easy-rsa.vars` is set properly and placed in `.config` container volume as `easy-rsa.vars`. 

In this case your custom EasyRSA options will be applied on the first OpenVPN Server start post PKI init step.

Default EasyRSA configuration can be set in `~/ElectromechVPN-server/config/easy-rsa.vars` file:
...
```
In the process of installation these vars will be copied to container volume `/etc/ElectromechVPN/pki/vars` and used during all EasyRSA operations.
...
The server config by default [specifies](https://github.com/d3vilh/ElectromechVPN-server/tree/main/config/server.conf#L35) `push redirect-gateway def1 bypass-dhcp`, meaning that after establishing the VPN connection, all traffic will go through the VPN. This might cause problems if you use local DNS recursors which are not directly reachable, since you will try to reach them through the VPN and they might not answer to you. If that happens, use public DNS resolvers like those of OpenDNS (`208.67.222.222` and `208.67.220.220`) or Google (`8.8.4.4` and `8.8.8.8`).

If you wish to use your local DNS server (Pi-Hile?), you have to modify a [dns-configuration](https://github.com/d3vilh/ElectromechVPN-server/tree/main/config/server.conf#L21) with your local DNS IP address. 
...
By default [d3vilh/ElectromechVPN-server](https://github.com/d3vilh/ElectromechVPN-server) OpenVPN server uses option `server 10.0.70.0/24` as **"Trusted"** subnet to grab dynamic IPs for all your Clients which, by default will have full access to your **"Private/Home"** subnet, as well as Internet over VPN.
However you can be desired to share internet over VPN with specific, Guest Clients and restrict access to your **"Private/Home"** subnet. For this scenario [d3vilh/ElectromechVPN-server](https://github.com/d3vilh/ElectromechVPN-server) `server.conf` configuration file has special `route 10.0.71.0/24` option, aka **"Guest users"** subnet.
...
By default `docker_entrypoint.sh` of [d3vilh/ElectromechVPN-server](https://github.com/d3vilh/ElectromechVPN-server) OpenVPN Server container will apply following Firewall rules:
...
You can apply optional Firewall rules in `~/ElectromechVPN-server/fw-rules.sh` file, which will be executed on the container start. 
...
Check detailed subnets description on [here](https://github.com/d3vilh/ElectromechVPN-ui/tree/main#ElectromechVPN-client-subnets-guest-and-home-users).
...
<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-ext_serv_ip1.png" alt="Configuration > Settings" width="350" border="1" />
...
<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-ext_serv_ip2.png" alt="Server Address" width="350" border="1" />  <img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-New_Client.png" alt="Create Certificate" width="350" border="1" />

To download .OVPN client configuration file, press on the `Client Name` you just created:

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-New_Client_download.png" alt="download OVPN" width="350" border="1" />

Install [Official OpenVPN client](https://ElectromechVPN.net/vpn-client/) to your client device.

Deliver .OVPN profile to the client device and import it as a FILE, then connect with new profile to enjoy your free VPN:

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Palm_import.png" alt="PalmTX Import" width="350" border="1" /> <img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Palm_connected.png" alt="PalmTX Connected" width="350" border="1" />
...
<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Cert-Renew.01.png" alt="Renew OpenVPN Certificate" width="600" border="1" />
...
<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Revoke.png" alt="Revoke Certificate" width="600" border="1" />
...
<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Restart.png" alt="OpenVPN Restart" width="600" border="1" />
...
Starting from vestion `0.9.3` ElectromechVPN-UI has Two Factor Authentication (2FA) feature.
ElectromechVPN-UI uses [oath-toolkit](https://savannah.nongnu.org/projects/oath-toolkit/) for two factor authentication. Means you don't need any ThirdParty 2FA provider.
When generating 2FA-enabled certificates ElectromechVPN-UI will provide QR code with 2FA secret, which you can scan with your 2FA app (Google Authenticator [iOS](https://apps.apple.com/us/app/google-authenticator/id388497605), [Android](https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2&pcampaignid=web_share), Microsoft Authenticator [iOS](https://apps.apple.com/us/app/microsoft-authenticator/id983156458), [Android](https://play.google.com/store/apps/details?id=com.azure.authenticator&pcampaignid=web_share), etc) to get 2FA token for connection with this certificate.
...
<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-2FA-Cert-Create.png" alt="2FA Certificate create" width="600" border="1" />
...
<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-2FA-Cert-details.png" alt="2FA Certificate details" width="600" border="1" />
...
<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-2FA-mobi-authenticator.png" alt="2FA Authenticator" width="350" border="1" />

Then you have to download and deliver `.OVPN profile` to [OpenVPN Connect app](https://ElectromechVPN.net/client/) and open it as a file. Following window appear:

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-2FA-mobi-profile-add.png" alt="2FA OpenVPN Connect profile add" width="350" border="1" />
...
<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-2FA-mobi-username.png" alt="2FA OpenVPN Connect profile username" width="350" border="1" />

When you'll be prompted to Enter the password, you have to enter your 2FA token from your 2FA app:

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-2FA-mobi-password.png" alt="2FA OpenVPN Connect profile 2FA password" width="350" border="1" />
...
<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-2FA-mobi-password-cert.png" alt="2FA OpenVPN Connect profile 2FA and Certificate passwords" width="350" border="1" />

  </details>

### Authentication with Google APIs (OAuth 2.0)
Starting from vestion `0.9.5.5` ElectromechVPN-UI support [OAuth 2.0 authentication with Google APIs](https://developers.google.com/identity/protocols/oauth2).
...
Kudos to [opsnin](https://github.com/d3vilh/ElectromechVPN-ui/pull/89) for this feature.
### User Management
...
<details>
      <summary>How to manage ElectromechVPN-UI Users</summary>

This functionality available via `"Users Profiles"` page:

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-ProfileAdmin.png" alt="Username > Profile" width="350" border="1" />


Then, if your user have enough privilegies you can Create new profile or manage profiles of other users:

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-ProfileCreate.png" alt="New OpenVPN UI Profile creation" width="600" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-ProfileManage.png" alt="OpenVPN UI Profiles management" width="600" border="1" />

</details>

### Screenshots:

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Login.png" alt="ElectromechVPN-UI Login screen" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Home.png" alt="ElectromechVPN-UI Home screen" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Certs.png" alt="ElectromechVPN-UI Certificates screen" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Create-Cert.png" alt="ElectromechVPN-UI Create Certificate screen" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Certs-Details-Expire.png" alt="ElectromechVPN-UI Expire Certificate details" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Certs-Details_OK.png" alt="ElectromechVPN-UI OK Certificate details" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-EasyRsaVars.png" alt="ElectromechVPN-UI EasyRSA vars screen" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-EasyRsaVars-View.png" alt="ElectromechVPN-UI EasyRSA vars config view screen" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Maintenance.png" alt="ElectromechVPN-UI Maintenance screen" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Server-config.png" alt="ElectromechVPN-UI Server Configuration screen" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Server-config-edit.png" alt="ElectromechVPN-UI Server Configuration edit screen" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-ClientConf.png" alt="ElectromechVPN-UI Client Configuration screen" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Config.png" alt="ElectromechVPN-UI Configuration screen" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Profile.png" alt="ElectromechVPN-UI User Profile" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-ProfileCreate.png" alt="New OpenVPN UI Profile creation" width="1000" border="1" />

<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-ProfileManage.png" alt="OpenVPN UI Profiles management" width="1000" border="1" />


<img src="https://github.com/d3vilh/ElectromechVPN-ui/blob/main/docs/images/ElectromechVPN-UI-Logs.png" alt="ElectromechVPN-UI Logs screen" width="1000" border="1" />

## Дякую and Kudos to the initiator of this project

Kudos to @adamwalach for development of the original [OpenVPN-WEB-UI](https://github.com/adamwalach/ElectromechVPN-web-ui) project which was used as solid foundation for OpenVPN UI.
