...
import (
	...
	mi "github.com/d3vilh/ElectromechVPN-server-config/server/mi"
	"github.com/d3vilh/ElectromechVPN-ui/lib"
	"github.com/d3vilh/ElectromechVPN-ui/models"
...
func (c *OVClientConfigController) Post() {
	...
	err := clientconfig.SaveToFile(filepath.Join(c.ConfigDir, "ElectromechVPN-client-config.tpl"), cfg.Config, destPath)
	if err != nil {
		logs.Warning(err)
		flash.Error(err.Error())
		flash.Store(&c.Controller)
