...
import (
	...
	clientconfig "github.com/d3vilh/ElectromechVPN-server-config/client/client-config"
	easyrsaconfig "github.com/d3vilh/ElectromechVPN-server-config/easyrsa/config"
	"github.com/d3vilh/ElectromechVPN-server-config/server/config"
...
func CreateDefaultOVConfig(configDir string, ovConfigPath string, address string, network string) {
	c := OVConfig{
		...
		Config: config.Config{
			...
			OVConfigClientConfigDir:  "/etc/ElectromechVPN/staticclients",
			...
			OVConfigLogfile:          "/var/log/ElectromechVPN/ElectromechVPN.log",
			OVConfigLogVerbose:       3,
			OVConfigStatusLog:        "/var/log/ElectromechVPN/ElectromechVPN-status.log",
	...
	if created, _, err := o.ReadOrCreate(&c, "Profile"); err == nil {
		...
		if _, err = os.Stat(serverConfig); os.IsNotExist(err) {
			if err = config.SaveToFile(filepath.Join(configDir, "ElectromechVPN-server-config.tpl"), c.Config, serverConfig); err != nil {
				logs.Error(err)
			}
		}
	} else {
...
func CreateDefaultOVClientConfig(configDir string, ovConfigPath string, address string, network string) {
	c := OVClientConfig{
		...
		Config: clientconfig.Config{
			...
			TFAIssuer:         "MFA%20ElectromechVPN-UI", // 2FA issuer
	...
	if created, _, err := o.ReadOrCreate(&c, "Profile"); err == nil {
		...
		if _, err = os.Stat(clientConfig); os.IsNotExist(err) {
			if err = clientconfig.SaveToFile(filepath.Join(configDir, "ElectromechVPN-client-config.tpl"), c.Config, clientConfig); err != nil {
				logs.Error(err)
			}
		}
	} else {
