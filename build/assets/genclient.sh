...
if  [[ -z $CERT_NAME ]]; then
    ...
    echo "User with name $CERT_NAME already exists under ElectromechVPN/clients. Exiting..."
    exit 1
fi
...
if [[ ! -z $TFA_NAME ]] && [[ $TFA_NAME != "none" ]]; then
    ...
    # QR code for user to pass to Google Authenticator or ElectromechVPN-UI
