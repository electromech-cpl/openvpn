...
# d3vilh/ElectromechVPN-server drafted 2FA support
#
# MFA verification by OpenVPN server using oath-tool
# This should be part of OpenVPN server container (github.com/d3vilh/ElectromechVPN-server). 
# This particular script is just example and does not use by ElectromechVPN-UI at all.

# VARIABLES
PASSFILE=$1    # Password file passed by ElectromechVPN-server with "auth-user-pass-verify /opt/app/bin/oath.sh via-file" in server.conf
OPENVPN_DIR=/etc/ElectromechVPN
OATH_SECRETS=$OPENVPN_DIR/clients/oath.secrets
LOG_FILE=/var/log/ElectromechVPN/oath.log
