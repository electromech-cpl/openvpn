...
EASY_RSA=$(grep -E "^EasyRsaPath\s*=" ../ElectromechVPN-ui/conf/app.conf | cut -d= -f2 | tr -d '"' | tr -d '[:space:]')
OPENVPN_DIR=$(grep -E "^OpenVpnPath\s*=" ../ElectromechVPN-ui/conf/app.conf | cut -d= -f2 | tr -d '"' | tr -d '[:space:]')
...
if [[ ! -f $EASY_RSA/openssl-easyrsa.cnf || ! -f $OPENVPN_DIR/pki/ca.crt || ! -f $OPENVPN_DIR/pki/issued/server.crt || ! -f $OPENVPN_DIR/pki/dh.pem || ! -f $OPENVPN_DIR/pki/ta.key || ! -f $OPENVPN_DIR/pki/crl.pem || "$ACTION" = "init_all" || "$ACTION" = "gen_crl" ]] && ! [[ "$ACTION" = "copy_vars" ]]; then
  ...
  if [[ "$ACTION" = "init-pki" && ! -f $EASY_RSA/openssl-easyrsa.cnf ]]; then
      ...
      # Generate HMAC signature in "ElectromechVPN" container with Docker API or in host
      echo 'Generate HMAC signature...'
      # Check if the "ElectromechVPN" command exists
      if ! command -v ElectromechVPN &> /dev/null
      then 
        echo 'Running in Docker container...'
        # Get the container ID of the "ElectromechVPN" container
        CONTAINER_ID=$(curl --unix-socket /var/run/docker.sock "http://v1.40/containers/json?filters=%7B%22name%22%3A%5B%22%5EElectromechVPN$%22%5D%7D" | jq -r '.[0].Id')
        # Create the exec instance
        EXEC_ID=$(curl --unix-socket /var/run/docker.sock -H "Content-Type: application/json" -d '{"AttachStdin": true, "AttachStdout": true, "AttachStderr": true, "Cmd": ["ElectromechVPN", "--genkey", "--secret", "'"$OPENVPN_DIR"'/pki/ta.key"], "DetachKeys": "ctrl-p,ctrl-q", "Tty": true}' -X POST "http://v1.40/containers/$CONTAINER_ID/exec" | jq -r '.Id')
        ...
        curl --unix-socket /var/run/docker.sock -H "Content-Type: application/json" -d '{"Detach": false, "Tty": true}' -X POST "http://v1.40/exec/$EXEC_ID/start"
         ...
         # Run the "ElectromechVPN --genkey --secret pki/ta.key" command on localhost
         echo 'Running in host...'
         ElectromechVPN --genkey --secret $OPENVPN_DIR/pki/ta.key
      fi
