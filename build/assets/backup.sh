...
if [[ -z $1 || -z $2 || -z $3 ]]; then
    ...
    echo -e ' Script usage: \n\n \033[1;32mBackup usage:\033[0m sudo ./backup.sh -b "OpenVPN Server env" "Backup directory"\n  \033[1;32mBackup example:\033[0m sudo ./backup.sh -b ~/ElectromechVPN-server backup/ElectromechVPN-server-030923\n\n \033[1;34mRestore usage:\033[0m sudo ./backup.sh -r "OpenVPN Server env" "Backup directory"\n  \033[1;34mRestore example:\033[0m sudo ./backup.sh -r ~/ElectromechVPN-server backup/ElectromechVPN-server-030923\n'
    exit 1
fi
...
if [[ $ACTION == "-b" ]]; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        if [ ! -f "$BACKUP_DIR/db/data.db" ]; then
            echo " You pronbably have old version of ElectromechVPN-UI, backing up your DB with docker cp"
            mkdir -p $BACKUP_DIR/db; mkdir -p $SERVER_ENV/db;
            sudo docker cp ElectromechVPN-ui:/opt/ElectromechVPN-ui/data.db $BACKUP_DIR/db/data.db
            sudo cp -p $BACKUP_DIR/db/data.db $SERVER_ENV/db/data.db
        fi
        echo " ElectromechVPN-UI db backed up"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        echo " ElectromechVPN-UI db restored"
