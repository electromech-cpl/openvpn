...
printf "ElectromechVPN-UI and qrencode were built \n\033[1;34mBuilding ElectromechVPN-UI image.\033[0m\n"

time docker run \
    -v "$PWD/../":/go/src/github.com/electromech-cpl/ElectromechVPN \
    -e GO111MODULE='auto' \
    -e CGO_ENABLED=1 \
    --rm \
    -w /usr/src/myapp \
    local/beego-v8 \
sh -c "cd /go/src/github.com/electromech-cpl/ElectromechVPN/ && \
    git config --global --add safe.directory /go/src/github.com/electromech-cpl/ElectromechVPN && \
    ...
    cp -p /app/qrencode/qrencode /go/src/github.com/electromech-cpl/ElectromechVPN/"
printf "ElectromechVPN-UI and qrencode were built \n\033[1;34mBuilding ElectromechVPN-UI image.\033[0m\n"

# Build ElectromechVPN-UI image
QRFILE="qrencode"
UIFILE="electromechvpn-ui.tar.gz"
cp -f ../$QRFILE ./
cp -f ../$UIFILE ./

# Build ElectromechVPN-UI image
docker build -t local/electromechvpn-ui .
